...
def eqs_dry(sur,avg,bott,D):
    ...
    while abs(dat2-dat1)>0.0005:
        ...
        if (f1*f2)<=0:
            ...
            dat1 = (dat1+temp2)/2.      
    try:
        dao1 = math.log(dat1/(math.exp(dat1*bott)-math.exp(dat1*sur)))+1
    except Exception:
        pass
    # loop through the column depth at 5 cm increments                
...
if sur < avg and avg < bott:
    # Dry Case
    profile = eqs_dry(sur,avg,bott,D)
    ...
    print('**********************************')
    print('ERROR :: Incorrect inputs for dry case, please re-enter values!')
    print('**********************************')
    sys.exit()
